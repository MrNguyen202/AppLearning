...
const userModels = {
    checkLogin: async (email: string, password: string) => {
        try {
          const response = await axios.post(
            "http://192.168.102.155:8080/api/v1/users/login",
            {
              email,
              password,
            },
    ...
    getUserById: async (userId: string) => {
        try {
            const response = await axios.get(`http://192.168.102.155:8080/api/v1/users/${userId}`);
            return response.data;
        } catch (error: any) {
            if (error.response) {
                console.error('Lỗi từ server:', error.response.data.message);
    ...
    updateUser: async (data: any) => {
        try {
            const response = await axios.put(`http://192.168.102.155:8080/api/v1/users/update`, data);
            return response.data;
        } catch (error: any) {
            if (error.response) {
                console.error('Lỗi từ server:', error.response.data.message);
    ...
    register: async (email: string, password: string, role: boolean) => {
      try {
        const response = await axios.post(
          "http://192.168.102.155:8080/api/v1/users/register",
          {
            email,
            password,
            role,
